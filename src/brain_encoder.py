...
class BrainEncoder(nn.Module):
    def __init__(self, fmri_masks_path, input_length=525, num_channels=61, latent_dim=4096, embed_dim=1024, patch_size=16, in_chans=1):
        super().__init__()
        self.RidgeRegression = RidgeRegression(fmri_masks_path, latent_dim)
        self.fMRIEncoder = fMRIEncoder(latent_dim=latent_dim, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
